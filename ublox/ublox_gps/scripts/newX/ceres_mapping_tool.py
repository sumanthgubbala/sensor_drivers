...
class OutdoorMapping:
    def __init__(self):
        rospy.init_node("outdoor_mapping", anonymous=True)
    ...
    def save_map_files(self, map_name):
        ...
        try:
            ...
            # Try changing permissions if the parent directory exists
            parent_dir = os.path.dirname(map_dir)
            try:
                os.chmod(parent_dir, 0o777)  # make parent writable
                os.makedirs(map_dir, exist_ok=True)
            except Exception as e:
                print(f"Still no permission to create {map_dir}: {e}")
