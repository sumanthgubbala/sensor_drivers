...
class OutdoorMapping:
    def __init__(self):
        ...
        self.map2odomRad = rospy.get_param("/map2odomRad", 0.0)
        ...
        self.current_map_name = rospy.get_param("/current_map_name", "ceres_farm_map")
        self.default_map_name = rospy.get_param("/default_map_name", "ceres_farm_map")
        ...
        rospy.loginfo(f"Parameters :\n"
                      ...
                      f"current_map_name: {self.current_map_name}\n"
                      f"default_map_name: {self.default_map_name}\n"
        ...
        original_boundary_location = os.path.join(self.map_save_directory, self.current_map_name)
        default_boundary_location = os.path.join(self.map_save_directory, self.default_map_name)
        os.makedirs(original_boundary_location, exist_ok=True)
        os.makedirs(default_boundary_location, exist_ok=True)
    ...
    def check_params_update(self, event):
        ...
        if new_map2odom != self.map2odom:
            ...
            self.map2odomRad = math.radians(new_map2odom)
            self.update_rotated_positions()
    ...
    def save_map_files(self):
        ...
        map_dir = os.path.join(self.map_save_directory, self.current_map_name)
        default_map_dir = os.path.join(self.map_save_directory, self.default_map_name)
        ...
        if save_map is not None:
            ...
            pgm_filename_default_dir = os.path.join(default_map_dir, "ceres_farm_map.pgm")
            ...
            yaml_filename_default_dir = os.path.join(default_map_dir, "ceres_farm_map.yaml")
            cv2.imwrite(pgm_filename, save_map)
            cv2.imwrite(pgm_filename_dir, save_map)
            cv2.imwrite(pgm_filename_default_dir, save_map)
            ...
            with open(yaml_filename_default_dir, 'w') as yaml_file_default_dir:
                yaml_file_default_dir.write(f"image: ceres_farm_map.pgm\n")
                yaml_file_default_dir.write(f"resolution: {self.map_resolution}\n")
                yaml_file_default_dir.write(f"origin: [{save_origin_x}, {save_origin_y}, 0.0]\n")
                yaml_file_default_dir.write("negate: 0\noccupied_thresh: 0.65\nfree_thresh: 0.196\n")

    def save_map2odom(self):
        map_dir = os.path.join(self.map_save_directory, self.current_map_name)
        map_default_dir = os.path.join(self.map_save_directory, self.default_map_name)
        map2odom_yaml = os.path.join(map_dir, "map2odom.yaml")
        map2odom_default_yaml = os.path.join(map_default_dir, "map2odom.yaml")
        with open(map2odom_yaml, 'w') as m2ofile:
            m2ofile.write(f"map2odom: {self.map2odom}\n")
            m2ofile.write(f"map2odomRad: {self.map2odomRad}\n")
        with open(map2odom_default_yaml, 'w') as m2odfile:
            m2odfile.write(f"map2odom: {self.map2odom}\n")
            m2odfile.write(f"map2odomRad: {self.map2odomRad}\n")
    
    def save_geofence(self):
        map_dir = os.path.join(self.map_save_directory, self.current_map_name)
        map_default_dir = os.path.join(self.map_save_directory, self.default_map_name)
        os.makedirs(map_dir, exist_ok=True)
        os.makedirs(map_default_dir, exist_ok=True)
        rotated_csv_filename = os.path.join(map_dir, "safe_boundary.csv")
        rotated_csv_default_filename = os.path.join(map_default_dir, "safe_boundary.csv")
        ...
        with open(rotated_csv_default_filename, 'w', newline='') as defaultcsvfile:
            writer2 = csv.writer(defaultcsvfile)
            writer2.writerows(sparse_geofence_data)
