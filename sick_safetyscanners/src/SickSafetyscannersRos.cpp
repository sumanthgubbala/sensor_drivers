  ...
  , m_use_pers_conf(false)
{
  ...
  // Flux Auto
  m_laser_contamination = m_nh.advertise<std_msgs::Bool>("sick/contamination",10);
  m_extended_laser_scan_publisher = m_nh.advertise<sick_safetyscanners::ExtendedLaserScanMsg>("extended_laser_scan", 100);
...
SickSafetyscannersRos::createGeneralSystemStateMessage(const sick::datastructure::Data& data)
{
  ...
  if (!data.getGeneralSystemStatePtr()->isEmpty())
  {
    ...
    Contamination_bool_.data     = msg.contamination_warning;
    m_laser_contamination.publish(Contamination_bool_);
